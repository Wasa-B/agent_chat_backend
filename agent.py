...
# LLM 추론 엔진으로 사용하여 어떤 작업을 수행할지,
# 해당 작업을 수행하는데 필요한 입력은 무엇인지 판단하는 모듈 - ReAct
...
# Agent 생성 부분을 try-catch 문으로 예외처리
agent = None
try:
...
# 답변 히스토리 누적 함수
async def process_query(query, conversation_history : list):
...
# 메인 함수
async def main():
  print("법률 관련 질문에 답변해 드립니다. 종료는 'q'를 입력해 주세요. ")
